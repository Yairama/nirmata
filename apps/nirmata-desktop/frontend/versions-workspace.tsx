...
function SnapshotCard({ title, snapshot }: { title: string; snapshot: ManualReviewObjectSnapshot | null }) {
  if (!snapshot) return <article className="version-object-snapshot empty"><h4>{title}</h4><p>El objeto no existe en esta versión.</p></article>;
  return (
    <article className="version-object-snapshot">
      <p className="panel-eyebrow">{title}</p>
      <h4>{snapshot.title}</h4>
      <dl className="version-field-list">
...
function AuditOperation({ operation, revisionId, variantId }: { operation: RevisionAuditOperationSnapshot; revisionId: string; variantId: string }) {
  return (
    <article className="version-audit-operation">
      <header>
        <div><p className="panel-eyebrow">{humanize(operation.source)}</p><h4>{operation.after?.title ?? operation.before?.title ?? "Objeto modificado"}</h4></div>
        <button type="button" className="ghost" onClick={() => void selectUriInScope(operation.targetUri, { variantId, revisionId })}>Abrir en esta versión</button>
      </header>
...
export function VersionsWorkspace() {
  ...
  return (
    <section className="versions-workspace" aria-labelledby="versions-title">
      <header className="versions-heading">
        <div><p className="panel-eyebrow">Linajes editoriales del mundo</p><h1 id="versions-title" tabIndex={-1}>Versiones</h1><p>Explora alternativas, compara su canon y prepara cambios sin alterar la variante de origen.</p></div>
        <dl className="versions-scope-summary">
      ...
      {(summaries.isLoading || history.isLoading) && <p className="notice info">Cargando versiones…</p>}
      {(summaries.isError || history.isError) && <p className="notice warning" role="alert">No se pudo cargar el historial de versiones. Reintenta sin cerrar el mundo.</p>}

      <section className="versions-section" aria-labelledby="variant-list-title">
        <div className="versions-section-heading"><div><p className="panel-eyebrow">Variantes</p><h2 id="variant-list-title">Líneas del mundo</h2></div>{session.read_only && <button type="button" onClick={() => void run(async () => { await viewActiveVersion(); await refresh(); })}>Volver a la versión actual</button>}</div>
        <div className="variant-card-grid">
          {variants.map((summary) => {
            ...
            return (
              <article className={`variant-card${summary.variant.archived ? " archived" : ""}`} key={summary.variant.id}>
                <header><div><h3>{summary.variant.name}</h3><div className="badge-row">{isActive && <span className="badge ready">Activa para escribir</span>}{isObserved && <span className="badge info">En pantalla</span>}{summary.variant.archived && <span className="badge context">Archivada</span>}</div></div></header>
                <dl className="variant-card-meta">
                ...
                {!summary.variant.archived && <div className="variant-card-actions">{!isActive && <button type="button" disabled={busy} onClick={() => void run(async () => { if (await switchWritingVariant(summary.variant.id)) await refresh(); })}>Escribir aquí</button>}{!isObserved && <button type="button" className="secondary" disabled={busy} onClick={() => void run(async () => { await observeScope({ variantId: summary.variant.id, revisionId: null }); await refresh(); })}>Ver última versión</button>}{!isActive && (archiveConfirm === summary.variant.id ? <><p>Esta línea tiene descendientes o importaciones. Archivarla no los elimina.</p><button type="button" className="danger" onClick={() => void archiveVariant(summary, true)}>Confirmar archivo</button><button type="button" className="ghost" onClick={() => setArchiveConfirm(null)}>Cancelar</button></> : <button type="button" className="ghost" onClick={() => void archiveVariant(summary)}>Archivar</button>)}</div>}
              </article>
            );
          })}
        </div>
        <div className="version-admin-grid">
          <form onSubmit={(event) => { event.preventDefault(); void createVariant(); }}><h3>Crear desde la versión seleccionada</h3><p>Origen: {selectedRevision?.summary ?? observed?.latestSummary ?? "Versión actual"}. La variante donde escribes no cambiará.</p><label>Nombre<input name="new-variant-name" autoComplete="off" value={newVariantName} maxLength={80} required onChange={(event) => setNewVariantName(event.currentTarget.value)} /></label><button disabled={busy} type="submit">Crear variante</button></form>
          <form onSubmit={(event) => { event.preventDefault(); void renameActive(); }}><h3>Renombrar línea activa</h3><p>Actualmente: {session.active_variant.name}</p><label>Nuevo nombre<input name="active-variant-name" autoComplete="off" value={renameValue} maxLength={80} required onChange={(event) => setRenameValue(event.currentTarget.value)} /></label><button disabled={busy} className="secondary" type="submit">Renombrar</button></form>
        </div>
      </section>

      <section className="versions-section" aria-labelledby="comparison-title">
        <div className="versions-section-heading"><div><p className="panel-eyebrow">Comparar y traer</p><h2 id="comparison-title">Dos líneas, una decisión explícita</h2></div></div>
        <div className="comparison-controls"><label>Comparar con<select value={selectedVariantId} onChange={(event) => { setSelectedVariantId(event.currentTarget.value); setComparison(null); }}>{variants.filter((item) => !item.variant.archived && item.variant.id !== session.read_scope.variantId).map((item) => <option key={item.variant.id} value={item.variant.id}>{item.variant.name}</option>)}</select></label><button type="button" className="secondary" disabled={!selectedVariant || busy} onClick={() => void compareSelected()}>Mostrar diferencias</button></div>
        {selectedVariant && <div className="merge-intent"><div><strong>Traer cambios de {selectedVariant.variant.name} hacia {session.active_variant.name}</strong><p>{session.active_variant.name} recibirá una propuesta revisable. {selectedVariant.variant.name} permanecerá sin cambios.</p></div><button type="button" disabled={busy || session.read_only || selectedVariant.variant.id === session.active_variant.id} onClick={() => void prepareMerge()}>Preparar en Cambios</button></div>}
        {session.read_only && <p className="notice warning">Estás viendo una versión de solo lectura. Vuelve a la versión actual antes de preparar cambios.</p>}
        {comparison && <div className="variant-comparison" aria-live="polite"><header className="comparison-side-headings"><div><span>Versión observada</span><strong>{observed?.variant.name}</strong></div><div><span>Versión comparada</span><strong>{selectedVariant?.variant.name}</strong></div></header>{comparison.differences.length === 0 ? <p className="empty-state">No hay diferencias entre estas versiones.</p> : comparison.differences.map((difference) => <article className="comparison-difference" key={objectUri(difference.objectRef)}><header><div><p className="panel-eyebrow">{humanize(difference.kind)}</p><h3>{differenceTitle(difference)}</h3></div><span>{difference.affectedReferences.length} referencia{difference.affectedReferences.length === 1 ? "" : "s"} afectada{difference.affectedReferences.length === 1 ? "" : "s"}</span></header><div className="comparison-fields">{changedFields(difference).map((field) => <div className="comparison-field" key={field.key}><h4>{humanize(field.key)}</h4><p><span>{observed?.variant.name}</span>{field.before}</p><p><span>{selectedVariant?.variant.name}</span>{field.after}</p></div>)}</div><div className="comparison-provenance"><p><strong>{observed?.variant.name}:</strong> {sourceLabel(difference.leftSource)}</p><p><strong>{selectedVariant?.variant.name}:</strong> {sourceLabel(difference.rightSource)}</p></div>{difference.affectedReferences.length > 0 && <div className="comparison-references"><strong>Referencias afectadas</strong>{difference.affectedReferences.map((reference) => <span key={objectUri(reference)}>{affectedReferenceLabel(reference)}</span>)}</div>}<div className="variant-card-actions"><button type="button" className="ghost" disabled={difference.before === null} onClick={() => void selectUriInScope(objectUri(difference.objectRef), difference.leftScope)}>Abrir en {observed?.variant.name}</button><button type="button" className="ghost" disabled={difference.after === null} onClick={() => void selectUriInScope(objectUri(difference.objectRef), difference.rightScope)}>Abrir en {selectedVariant?.variant.name}</button></div></article>)}</div>}
      </section>

      <section className="versions-section revision-timeline" aria-labelledby="history-title">
        <div className="versions-section-heading"><div><p className="panel-eyebrow">Historia editorial</p><h2 id="history-title">Versiones de {observed?.variant.name ?? session.active_variant.name}</h2><p>Deshacer solo está disponible para el último cambio lógico de la línea activa. Siempre crea una versión nueva y nunca borra historia.</p></div><label>Filtrar por objeto o resumen<input type="search" value={historyFilter} onChange={(event) => setHistoryFilter(event.currentTarget.value)} /></label></div>
        {revisions.length === 0 ? <p className="empty-state">Esta línea todavía no tiene cambios aplicados.</p> : <div className="revision-layout"><ol className="revision-list">{filteredRevisions.map((entry) => <li key={entry.revisionId}><button type="button" aria-current={entry.revisionId === selectedRevision?.revisionId ? "true" : undefined} onClick={() => setSelectedRevisionId(entry.revisionId)}><strong>{entry.summary}</strong><span>{formatTimestamp(entry.createdAtMs)} · {entry.operations.length} objeto{entry.operations.length === 1 ? "" : "s"}</span><span>{entry.isCurrentHead ? "Versión actual" : entry.isCurrentUndoTarget ? "Se puede deshacer" : entry.undoneRevisionId ? "Versión inversa" : "Versión anterior"}</span></button></li>)}</ol>{selectedRevision && <article className="revision-detail"><header><div><p className="panel-eyebrow">{humanize(selectedRevision.author)}</p><h3>{selectedRevision.summary}</h3><p>{formatTimestamp(selectedRevision.createdAtMs)} · {selectedRevision.operations.length} operación{selectedRevision.operations.length === 1 ? "" : "es"}</p></div><button type="button" className="secondary" disabled={!selectedRevision.isCurrentUndoTarget || busy} onClick={() => void undo(selectedRevision)}>Deshacer creando otra versión</button></header>{selectedRevision.undoneRevisionId && <p className="notice info">Esta versión invierte un cambio anterior y conserva ambos estados en la historia.</p>}{selectedRevision.operations.map((operation) => <AuditOperation key={operation.operationId} operation={operation} revisionId={selectedRevision.revisionId} variantId={session.read_scope.variantId} />)}</article>}</div>}
      </section>
    </section>
  );
}
